...
export function fetchBoundaryDetails(parentBoundaryId = 1) {
  return function(dispatch, getState) {
    ...
    return fetch(
      serverApiBase + "boundaries/?parent=" + parentBoundaryId + "&limit=500",
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
export function fetchStudentGroups(institutionId) {
  return function(dispatch, getState) {
    var url =
      serverApiBase + "institutions/" + institutionId + "/studentgroups/";
    return fetch(url, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
export const getStudents = (institutionId, classId) => {
  return get(
    `${serverApiBase}institutions/${institutionId}/studentgroups/${classId}/students/`
  );
};
...
export function fetchEntitiesFromServer(parentBoundaryId) {
  return function(dispatch, getState) {
    const state = getState();
    return dispatch(
      boundaryType(parentBoundaryId, state.boundaries.boundaryDetails)(
        parentBoundaryId
      )
    );
  };
